...
export const useLogin = (): {
    ...
    const login = useCallback(
        async (e: FormEvent) => {
            ...
            try {
                ...
                const message = error.code === 'auth/invalid-credential' ? 'Invalid email or password' : error.message || 'An unknown error occurred';
                toast.error(message);
    ...
    const loginWithGoogle = useCallback(async () => {
        ...
        try {
            ...
            const error = err as { code?: string; message?: string };
            const message = error.code === 'auth/popup-closed-by-user' ? 'Google sign-in was cancelled' : error.message || 'An unknown error occurred';
            toast.error(message);
            setErrors({ message, ...(error.code !== undefined && { code: error.code }) });
        } finally {
