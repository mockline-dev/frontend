/**
 * Backend Runner
 * Main orchestration for running backend servers
 */
...
import { getErrorMessage, createBackendRunError } from '@/types/errors';
import { mkdir, writeFile } from 'fs/promises';
import { dirname, join } from 'path';

/**
 * Run backend server for a project
 */
export const runBackend = async ({
    projectId,
    onLog
}: RunBackendParams): Promise<RunBackendResponse> => {
    ...
    try {
        ...
        for (const file of files) {
            try {
                ...
                pushLog(
                    'error',
                    `Failed to download file ${file.name}: ${errorMessage}`,
                    'downloader'
                );
                validation.errors.push(`Failed to download ${file.name}: ${errorMessage}`);
            }
        }
        ...
        try {
            ...
            if (existsSync(join(tempDir, 'requirements.txt'))) {
                ...
                let requirementsContent = await cleanRequirementsContent(requirementsPath);

                // Write cleaned requirements back with email-validator included
                await writeFile(requirementsPath, requirementsContent, 'utf-8');
                pushLog(
                    'info',
                    'Cleaned requirements.txt - removed version constraints and ensured email-validator is present',
                    'pip'
                );
                ...
                await runStreamingCommand(
                    ...
                    { cwd: tempDir, env: getSafeEnvironmentVariables() },
                    'pip',
                    undefined,
                    onLog
                );
                ...
                try {
                    await runStreamingCommand(
                        'python',
                        ['-c', 'import email_validator; print(f"email-validator version: {email_validator.__version__}")'],
                        { cwd: tempDir, env: getSafeEnvironmentVariables() },
                        'pip',
                        30000,
                        onLog
                    ...
                    pushLog(
                        'warning',
                        'email-validator validation failed, attempting to install it separately...',
                        'pip'
                    );
                    try {
                        await runStreamingCommand(
                            'pip',
                            ['install', 'email-validator', '--disable-pip-version-check'],
                            { cwd: tempDir, env: getSafeEnvironmentVariables() },
                            'pip',
                            60000,
                            onLog
                        ...
                        pushLog(
                            'success',
                            'email-validator installed successfully via separate installation',
                            'pip'
                        );
                    } catch (separateInstallError) {
                        const errorMessage = getErrorMessage(separateInstallError);
                        pushLog('error', `Failed to install email-validator: ${errorMessage}`, 'pip');
                ...
                await runStreamingCommand(
                    'pip',
                    [...DEFAULT_PYTHON_DEPENDENCIES, '--disable-pip-version-check'],
                    { cwd: tempDir, env: getSafeEnvironmentVariables() },
                    'pip',
                    undefined,
                    onLog
            ...
            const errorMessage = getErrorMessage(error);
            pushLog('error', `Failed to install dependencies: ${errorMessage}`, 'pip');
            await cleanupTempDir(tempDir);
            return {
                success: false,
        ...
        try {
            await validatePythonSyntax(mainRelativePath, tempDir);
            pushLog('success', 'Python syntax validation passed', 'python');
        } catch (error) {
            const errorMessage = getErrorMessage(error);
            pushLog('error', `Python syntax validation failed: ${errorMessage}`, 'python');
            await cleanupTempDir(tempDir);
            return {
                success: false,
        ...
        const serverProcess = spawnChild(
            'uvicorn',
            [`${moduleName}:app`, '--host', DEFAULT_SERVER_HOST, '--port', String(port)],
            {
                cwd: tempDir,
                shell: false,
        ...
        for (let attempt = 1; attempt <= HEALTH_CHECK_RETRIES; attempt++) {
            ...
            const healthCheckResult = await performHealthCheck(port, attempt);

            if (healthCheckResult.success) {
                serverRunning = true;
                pushLog(
                    'success',
                    `Server health check passed on attempt ${attempt}/${HEALTH_CHECK_RETRIES}`,
                    'uvicorn'
                );
                break;
            }
            ...
            pushLog(
                'warning',
                `Server health check attempt ${attempt}/${HEALTH_CHECK_RETRIES} failed: ${healthCheckResult.error}`,
                'uvicorn'
            );
        ...
        if (!serverRunning) {
            ...
            if (combinedError.includes('email-validator') || combinedError.includes('email_validator')) {
                ...
                combinedError.includes('modulenotfounderror')
            ) {
        ...
        pushLog('error', `Failed to run backend: ${errorMessage}`, 'backend-runner');
        return {
            success: false,
            error: errorMessage || 'Failed to run backend',
            data: {
                success: false,
